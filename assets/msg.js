...
(function () {
  ...
  function css() {
    ...
    s.textContent = [
      ...
      '.ms-b.me .ms-bb{background:var(--ink,#2b2740);color:var(--paper,#fff);border-bottom-right-radius:4px}',
      ...
      ' background:var(--ink,#2b2740);color:var(--paper,#fff);font-weight:700;font-size:12.5px}',
  ...
  async function report(uid) {
    ...
    /* ★★ 2026-08-20 · 돌려준 값을 <b>봅니다.</b>
       ─────────────────────────────────────────────────────────────
       oc_report_add 는 도배 막이에 걸리면 <b>오류가 아니라 0</b> 을
       돌려줍니다. 전에는 그것을 안 보고 늘 「신고했습니다」라고
       말했습니다 — <b>이용자가 신고했다고 믿는데 아무것도 안 남는</b>
       상황이었습니다. 괴롭힘을 당한 사람에게 그러면 안 됩니다.
       ★ 0 은 「이미 들어와 있다」는 뜻이지 잘못이 아닙니다. 그래서
         빨간 글씨가 아니라 <b>알림</b>으로 말합니다. */
    if (Number(r.data) === 0) {
      note('no', '조금 전에 <b>같은 신고가 이미 들어왔습니다.</b> 10분 뒤에'
        + ' 다시 하실 수 있습니다. 급하시면 <b>차단</b>을 누르십시오.');
      return;
    }
