...
/* 통합검색 패널 — 돋보기 클릭 시 열리고, 엔터로 /search.html 로 이동
   스타일은 여기서 넣는다. base.css 를 건드리지 않으므로 다른 페이지에 영향이 없고,
   헤더가 있는 모든 페이지에 한 번에 적용된다.
   위치는 헤더 높이를 읽어 계산하므로 헤더 CSS 구조를 몰라도 안전하다. */
(function(){
  if(document.getElementById('hsearchStyle'))return;
  var css=''
   +'.hsearch{position:fixed;left:0;right:0;z-index:90;background:var(--paper,#fff);'
   +'border-bottom:1px solid var(--line,#e4e4ec);box-shadow:0 10px 28px rgba(20,20,40,.10);'
   +'opacity:0;transform:translateY(-10px);pointer-events:none;transition:opacity .18s ease,transform .18s ease}'
   +'.hsearch.open{opacity:1;transform:none;pointer-events:auto}'
   +'.hsearch-form{max-width:760px;margin:0 auto;display:flex;align-items:center;gap:10px;padding:13px 20px}'
   +'.hsearch-ic{flex:0 0 auto;width:18px;height:18px;color:var(--text-3,#8a8c9e)}'
   +'.hsearch-form input{flex:1;min-width:0;height:44px;padding:0 4px;font-size:15.5px;'
   +'border:0;border-bottom:1.5px solid var(--line,#e4e4ec);background:transparent;'
   +'color:var(--text-1,#1b1c2b);outline:none;transition:border-color .15s}'
   +'.hsearch-form input:focus{border-bottom-color:var(--violet-3,#9C7FD6)}'
   +'.hsearch-form button[type=submit]{height:40px;padding:0 18px;font-size:14px;font-weight:700;'
   +'color:#fff;background:var(--text-1,#1b1c2b);border:0;border-radius:9px;cursor:pointer;white-space:nowrap}'
   +'.hsearch-form button[type=submit]:hover{background:var(--violet-3,#7C63B0)}'
   +'.hsearch-close{width:36px;height:36px;display:grid;place-items:center;border:0;'
   +'background:transparent;color:var(--text-3,#8a8c9e);cursor:pointer;border-radius:8px}'
   +'.hsearch-close:hover{background:var(--paper-2,#f2f2f7);color:var(--text-1,#1b1c2b)}'
   +'@media(max-width:640px){.hsearch-form{padding:11px 14px;gap:7px}'
   +'.hsearch-form button[type=submit]{padding:0 13px}}';
  var st=document.createElement('style');
  st.id='hsearchStyle'; st.textContent=css;
...
(function(){
  ...
  function place(){
    var sh=document.getElementById('siteHeader');
    var top=sh?Math.round(sh.getBoundingClientRect().bottom):64;
  ...
  function close(){
    ...
    setTimeout(function(){ if(!box.classList.contains('open')) box.hidden=true; },200);
  }
  ...
  window.addEventListener('scroll',function(){ if(box.classList.contains('open'))place(); },{passive:true});
  ...
  if(form){
    form.addEventListener('submit',function(e){
      ...
      if(!q){ e.preventDefault(); inp.focus(); return; }
      /* action=/search.html 이므로 기본 동작으로 이동한다 (JS 없이도 작동) */
    });
  }
  /* 검색 페이지에서는 패널을 열지 않고 본문 검색창을 쓴다 */
  if(location.pathname.replace(/\/+$/,'').endsWith('/search.html')){
    btn.addEventListener('click',function(){
      var main=document.querySelector('.osr-form input');
      if(main){ close(); main.focus(); }
    });
  }
})();
