...
function ocN(tpl) {
  ...
  function init(cfg) {
    ...
    /* ★★ 2026-08-11 · <b>held</b> 를 더했습니다 — 여러 말 대응 ★★
       ─────────────────────────────────────────────────────────
       예전에는 이렇게 이어 붙였습니다 —
           '오퍼스클램의 ' + what + ' 자료는 지금 ' + 숫자 + '건입니다.'

       ★ 무엇이 잘못됐나
         i18n 은 <b>글자 마디 단위</b>로 옮깁니다. 그런데 변수를 글자로
         이어 붙이면 <b>한 마디</b>가 됩니다 —
             「오퍼스클램의 음악인 자료는 지금 」
         이런 조합이 갈래마다 달라 <b>일곱 가지</b>가 되고, 사전에
         담을 수도 없어 <b>한국어가 그대로</b> 나왔습니다.

       ★ 왜 &lt;span&gt; 으로 쪼개는 것만으로는 안 되는가
         쪼개면 「오퍼스클램의 」 / 갈래 / 「 자료는 지금 」 / 숫자 /
         「건입니다.」 다섯 마디가 됩니다. 각각 옮길 수는 있지만
         <b>영어는 말차례가 달라</b> 「OPUSCLAM currently holds
         Musicians entries in 0 at this time.」 처럼 뒤엉킵니다.

       ★ 그래서 <b>숫자를 앞에 두고 나머지를 온전한 한 문장</b>으로
         만들었습니다 — 「15,222」 + 「건의 음악인 자료가 담겨 있습니다.」
         뒷문장이 통째로 사전에 있으므로 어느 말로도 자연스럽습니다. */
    var EMPTY_INFO = {
      persons: { what: '음악인', db: 'person', ask: '찾으시는 분이 목록에 없나요?',
                 held: '건의 음악인 자료가 담겨 있어요.',
                 tip: '(같은 분이 다른 표기로 담겨 있을 수 있어요)' },
      orgs: { what: '음악단체', db: 'org', ask: '찾으시는 단체가 없나요?',
              held: '건의 음악단체 자료가 담겨 있어요.',
              tip: '(「서울시립교향악단」 · 「Seoul Philharmonic」처럼 표기가 여럿이에요)' },
      venues: { what: '공연장', db: 'venue', ask: '찾으시는 공연장이 없나요?',
                held: '건의 공연장 자료가 담겨 있어요.',
                tip: '(「예술의전당 콘서트홀」처럼 홀 이름까지 넣거나, 반대로 빼고 찾아보셔도 좋아요)' },
      schools: { what: '음악학교', db: 'school', ask: '찾으시는 학교가 없나요?',
                 held: '건의 음악학교 자료가 담겨 있어요.',
                 tip: '(「예술고등학교」 · 「예고」처럼 줄임말로도 찾아보세요)' },
      modern_composers: { what: '현대음악 작곡가', db: 'modern', ask: '찾으시는 작곡가가 없나요?',
                          held: '건의 현대음악 작곡가 자료가 담겨 있어요.',
                          tip: '(원어 표기로도 한번 찾아보세요)' },
      foundations: { what: '기관·재단', db: 'foundation', ask: '찾으시는 기관이 없나요?',
                     held: '건의 기관·재단 자료가 담겨 있어요.',
                     tip: '(재단 · 협회 · 음반사 · 콩쿠르 주최를 함께 담고 있어요)' },
      academic: { what: '학술 자료', db: 'academic', ask: '찾으시는 자료가 없나요?',
                  held: '건의 학술 자료가 담겨 있어요.',
                  tip: '(제목 전체보다 낱말 하나로 찾는 편이 잘 걸려요)' },
    };

    function emptyHtml() {
      var key = cfg.entity || cfg.table;
      var info = EMPTY_INFO[key] || { what: '자료', db: 'person', ask: '찾으시는 자료가 없나요?',
                                      held: '건의 자료가 담겨 있어요.', tip: '' };
      ...
      /* ★ 검색어(kw)는 <b>사람이 넣은 말</b>이라 옮길 것이 아닙니다.
           그것을 감싼 &lt;b&gt; 는 남겨 둡니다 — 조각이 나뉘어도
           뒷부분('로 찾은 결과가 없습니다.')이 온전한 한 문장이므로
           사전에 담아 두면 그 부분은 제대로 옮겨집니다. */
      var head = kw
      ...
      return ''
        ...
        /* ★ 덧말을 &lt;span&gt; 으로 감쌉니다 — 감싸지 않으면 앞 문장과
             <b>한 마디</b>가 되어 갈래마다 조합이 달라집니다(일곱 가지).
             쪼개 두면 앞 문장과 덧말이 <b>각각</b> 옮겨집니다. */
        +     '<li>한글 · 영문 · 원어 표기를 바꿔 찾아보세요. '
        +       '<span>' + esc(info.tip) + '</span></li>'
        /* ★ 한국어 낱말을 &lt;b&gt; 로 감싸지 않습니다 — 문장이 조각나
             i18n 이 <b>일부만 옮겨</b> 한국어와 영어가 뒤섞입니다. */
        +     '<li>이름 전체보다 일부만 넣어 보세요. 그편이 잘 찾아져요.</li>'
        ...
        +     (state.filters
              ? '<li>검색칸 옆의 국내/외 · 분야 조건이 함께 걸려 있어요. 그것을 풀고 다시 찾아보세요.</li>'
              : '')
        +   '</ul>'
        ...
        /* ★★ 2026-08-11 · <b>한국어 낱말을 &lt;b&gt; 로 감싸지 않습니다</b> ★★
           ─────────────────────────────────────────────────────
           파트너가 「영문 화면에 이렇게 나온다」 고 알려 주셨습니다 —

               「자격을 갖춘 회원은 직접 등록·보강You may do so.」

           i18n 은 <b>글자 조각 단위</b>로 옮깁니다. 그런데 이 문장이
           &lt;b&gt; 때문에 <b>세 조각</b>으로 나뉘어 있었습니다 —

               ① '자격을 갖춘 회원은 직접 '   사전에 없음 → 한국어 그대로
               ② '등록·보강'                 사전에 없음 → 그대로
               ③ '하실 수 있습니다.'          <b>사전에 있음 → 영어로</b>

           그래서 마지막 조각만 영어가 되어 뒤섞였습니다.
           → &lt;b&gt; 를 없애 <b>한 문장</b>으로 둡니다. 그러면 통째로 옮겨집니다.

         ★ 숫자를 감싼 &lt;b&gt; 는 괜찮습니다 (아래 '건' 앞)
           숫자는 옮길 것이 아니고, 어차피 변수라 조각이 나뉩니다.
           <b>한국어 낱말</b>을 감쌀 때만 문제가 됩니다. */
        +       '자격을 갖춘 회원은 직접 등록·보강하실 수 있습니다. '
        +       '<span class="pdb-none-note">등록하신 자료는 관리자 확인을 거쳐 반영돼요.</span>'
        +     '</p>'
        +     '<div class="pdb-none-btns" id="pdbNoneBtns">'
        +       '<a class="pdb-none-btn" href="' + wr + '">'
        +         '<span>' + esc(info.what) + '</span> <span>등록·보강하기</span></a>'
    ...
    function fixEmptyCta() {
      ...
      sb.auth.getSession().then(function (r) {
        ...
        hint.textContent = '등록·보강은 전공자 · 음악관계자 · 단체 · 음악학교 회원에게 열려 있어요. '
          + '가입하실 때 회원 종류를 골라 주세요.';
